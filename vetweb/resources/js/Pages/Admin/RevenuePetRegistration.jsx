...
export default function RevenuePetRegistration() {
    ...
    // Fetch revenue data for Pet Registration only
    const fetchRevenueData = async () => {
        ...
        try {
            ...
            if (data.success) {
                // Filter for pet registration only (exclude VHC)
                const filteredData = {
                    ...data,
                    // Assuming the API returns separate counts or we filter VHC out
                    // For now, we show all data but label it as Pet Registration
                    monthly_revenue: data.monthly_revenue?.map(m => ({
                        ...m,
                        // Filter out VHC registrations if the API provides type breakdown
                    })) || [],
                };
    ...
    const handleUpdateFee = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setFeeReason('');
                fetchRevenueData();
                showFlash('success', 'Registration fee updated successfully!');
            } else {
                showFlash('error', data.message || 'Failed to update fee');
            }
        } catch (error) {
            console.error('Failed to update fee:', error);
            showFlash('error', 'Failed to update fee');
        }
    };
        ...
        <div className="flex min-h-screen bg-gray-50">
            ...
            <main className="flex-1 ml-72 p-8">
                <div className="max-w-7xl mx-auto">
                    {/* Header */}
                    <div className="mb-8">
                        <h1 className="text-2xl font-bold text-gray-900">Pet Registration Revenue</h1>
                        <p className="text-gray-500 mt-1">View and manage pet registration fees</p>
                    </div>
